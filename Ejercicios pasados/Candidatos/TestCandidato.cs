            ...
            switch (opc)
            {
                ...
                case 4:
                    while (intentos!=3)
                    {
                        ...
                        var totalVotos = candidatoUno.amount + candidatoDos.amount + candidatoTres.amount;
                        switch (pass)
                        {
                            case PASSWORD:
                                switch (cierre)
                                {
                                    case false when candidatoUno.amount > candidatoDos.amount && candidatoUno.amount > candidatoTres.amount:
                                        Console.WriteLine($"El ganador es {candidatoUno.name} con {candidatoUno.amount} votos [{(candidatoUno.amount * 100)/totalVotos}%]");
                                        Console.WriteLine($"{candidatoDos.name} obtuvo {candidatoDos.amount} votos [{(candidatoDos.amount * 100)/totalVotos}%]");
                                        Console.WriteLine($"{candidatoTres.name} obtuvo {candidatoTres.amount} votos [{(candidatoTres.amount * 100)/totalVotos}%]");
                                        cierre = true;
                                        intentos = 3;
                                        break;
                                    case false when candidatoDos.amount > candidatoUno.amount && candidatoDos.amount > candidatoTres.amount:
                                        Console.WriteLine($"El ganador es {candidatoDos.name} con {candidatoDos.amount} votos [{(candidatoDos.amount * 100)/totalVotos}%]");
                                        Console.WriteLine($"{candidatoUno.name} obtuvo {candidatoUno.amount} votos [{(candidatoUno.amount * 100)/totalVotos}%]");
                                        Console.WriteLine($"{candidatoTres.name} obtuvo {candidatoTres.amount} votos [{(candidatoTres.amount * 100)/totalVotos}%]");
                                        cierre = true;
                                        intentos = 3;
                                        break;
                                    case false when candidatoTres.amount > candidatoDos.amount && candidatoTres.amount > candidatoUno.amount:
                                        Console.WriteLine($"El ganador es {candidatoTres.name} con {candidatoTres.amount} votos [{(candidatoTres.amount * 100)/totalVotos}%]");
                                        Console.WriteLine($"{candidatoUno.name} obtuvo {candidatoUno.amount} votos [{(candidatoUno.amount * 100)/totalVotos}%]");
                                        Console.WriteLine($"{candidatoDos.name} obtuvo {candidatoDos.amount} votos [{(candidatoDos.amount * 100)/totalVotos}%]");
                                        cierre = true;
                                        intentos = 3;
